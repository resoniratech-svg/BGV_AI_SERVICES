...
from services.salary_slip_ocr_service import SalarySlipOCRService
...
@salary_slip_bp.route("/salary-slip/ocr", methods=["POST"])
def salary_slip_ocr():
    ...
    try:
        ...
        result = SalarySlipOCRService.verify_salary_slip(
...
def get_salary_slip_result(candidate_id):
    ...
    try:
        result = SalarySlipRepository.get_salary_slip_ocr_result(candidate_id)

        if not result:
            return jsonify(
                {"success": False, "message": "Salary Slip OCR result not found."}
            ), 404
