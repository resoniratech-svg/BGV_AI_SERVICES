...
class BankStatementService:
    ...
    @staticmethod
    def upload_bank_statement(
        candidate_id, bgv_id, bank_name=None, bank_statement_password=None
    ):
        ####################################################
        # UPLOADED BANK STATEMENT
        ####################################################
        ...
        if not callback_url:
            raise Exception("BANK_STATEMENT_CALLBACK_URL is not configured.")
        ...
        with open(document_path, "rb") as statement_file:
            ...
            ################################################
            # GRIDLINES UPLOAD API
            ################################################
            response = OnGridClient.post_multipart(
                "/bank-api/bank-statement-analyzer/upload", files, data
            )
        ...
        if response.get("status") != 200:
            raise Exception(
                ...
                f"HTTP Status :\n\n{response.get('status')}\n\n"
                f"Response :\n\n"
                f"{json.dumps(response, indent=4, default=str)}"
            )
        ...
        if data_payload.get("code") != "1019":
            raise Exception(
                data_payload.get("message", "Bank Statement Upload failed.")
            )
