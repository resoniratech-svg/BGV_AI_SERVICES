...
class OnGridPANService:
    ...
    @staticmethod
    def verify_pan(
        candidate_id, bgv_id, pan_ocr_result_id, pan_number, full_name, date_of_birth
    ):

        payload = {"pan_number": pan_number, "consent": "Y"}

        response = OnGridClient.post("/pan-api/fetch-detailed", payload)

        if not response:
            raise Exception("Empty PAN response")

        if response.get("status") != 200:
            raise Exception(response.get("message", "PAN verification failed"))

        code = response.get("data", {}).get("code")

        if code != "1000":
            raise Exception(
                response.get("data", {}).get("message", "PAN does not exist")
            )

        pan_data = response.get("data", {}).get("pan_data", {})
        ...
        provider_full_name = OnGridPANService.normalize_name(provider_full_name)

        ocr_name = OnGridPANService.normalize_name(full_name)

        pan_match_status = (
            "MATCH"
            if pan_number.upper() == provider_pan_number.upper()
            else "NOT_MATCH"
        )
        ...
        dob_match_status = (
            "MATCH"
            if str(date_of_birth).strip() == str(provider_dob).strip()
            else "NOT_MATCH"
        )
        ...
        ProviderUsageRepository.increment_usage(
            provider_name="GRIDLINES", verification_type="PAN"
        )
