...
class FaceMatchService:
    @staticmethod
    def verify_face(candidate_id, bgv_id, document_id):
        ...
        if not aadhaar:
            raise Exception("Aadhaar verification not found")

        resident_image = aadhaar.get("resident_image")

        if not resident_image:
            raise Exception("Resident image not available")
        ...
        if not document:
            raise Exception("Candidate selfie not found")

        selfie_path = os.path.abspath(document["file_path"])

        if not os.path.exists(selfie_path):
            raise Exception("Selfie file missing")

        ########################################
        # Convert resident image
        ########################################

        image_bytes = base64.b64decode(resident_image)
        ...
        aadhaar_path = temp_file.name

        ########################################
        # Gridlines request
        ########################################

        with open(aadhaar_path, "rb") as img1, open(selfie_path, "rb") as img2:
            files = {"file_1": img1, "file_2": img2}

            data = {"consent": "Y"}

            response = OnGridClient.post_multipart("/face-api/verify", files, data)
        ...
        if not response:
            raise Exception("Face match response empty")
        ...
        confidence = float(response.get("data", {}).get("confidence", 0))

        ########################################
        # Threshold
        ########################################

        verification_status = "MATCH" if confidence >= 0.70 else "NOT_MATCH"
