...
from services.ocr.aadhaar_ocr_service import AadhaarOCRService
...
class AadhaarVerificationService:
    @staticmethod
    def verify_aadhaar(candidate_id, bgv_id, document_id):

        # ==========================================
        # OCR EXTRACTION
        # ==========================================

        ocr_data = AadhaarOCRService.extract_aadhaar_data(
        ...
        if not full_name:
            raise Exception("Name not extracted from Aadhaar")

        if not date_of_birth:
            raise Exception("Date of birth not extracted from Aadhaar")

        if not gender:
            raise Exception("Gender not extracted from Aadhaar")

        # ==========================================
        # SAVE OCR RESULT
        # ==========================================

        aadhaar_ocr_result_id = AadhaarRepository.save_aadhaar_ocr_result(
            ...
            api_reference_id=ocr_data.get("request_id"),
            raw_response=json.dumps(ocr_data),
        )
        ...
        name_match_status = (
            "MATCH"
            if full_name.strip().upper() == resident_name.strip().upper()
            else "NOT_MATCH"
        )
        ...
        dob_match_status = (
            "MATCH" if date_of_birth.strip() == uidai_dob.strip() else "NOT_MATCH"
        )
        ...
        verification_status = (
            "VERIFIED"
            if (name_match_status == "MATCH" and dob_match_status == "MATCH")
            else "FAILED"
        )
        ...
        AadhaarRepository.save_aadhaar_verification_result(
            candidate_id=candidate_id,
            bgv_id=bgv_id,
            aadhaar_ocr_result_id=aadhaar_ocr_result_id,
