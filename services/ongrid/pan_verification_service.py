...
class OnGridPANVerificationService:
    ...
    def verify_pan(candidate_id, bgv_id, document_id):
        ...
        if not pan_data.get("pan_number"):
            raise Exception("PAN number not extracted")

        if not pan_data.get("full_name"):
            raise Exception("PAN holder name not extracted")
        ...
        pan_ocr_result_id = PanRepository.save_pan_ocr_result(
            ...
            raw_response=json.dumps(pan_data.get("raw_response")),
        )
        ...
        return OnGridPANService.verify_pan(
            candidate_id=candidate_id,
            bgv_id=bgv_id,
