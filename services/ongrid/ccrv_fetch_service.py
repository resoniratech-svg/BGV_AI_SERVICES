...
class CCRVFetchService:
    @staticmethod
    def fetch_report(transaction_id):
        ...
        CCRVRepository.mark_fetch_attempted(
            transaction_id=transaction_id, fetch_attempted_at=datetime.now()
        )
        ...
        response = OnGridClient.get(f"/ccrv-api/fetch-report/{transaction_id}")
        ...
        if not response:
            raise Exception("Empty CCRV Fetch response")
        ...
        if status != 200:
            CCRVRepository.update_request_failed(
                transaction_id=transaction_id, raw_response=json.dumps(response)
            )
        ...
        ccrv_status = data.get("ccrv_status")

        ####################################################
        # STILL PROCESSING
        ####################################################

        if ccrv_status in ("REQUESTED", "IN_PROGRESS"):
            ...
            return {"completed": False, "ccrv_status": ccrv_status}
        ...
        if ccrv_status == "FAILED":
            ...
            return {"completed": False, "ccrv_status": "FAILED"}
        ...
        if CCRVRepository.result_exists(request["id"]):
            CCRVRepository.delete_existing_result(request["id"])
        ...
        ccrv_result_id = CCRVRepository.save_ccrv_result(
        ...
        for case in cases:
            CCRVRepository.save_case(
                ...
                criminal_act_severity=case.get("criminal_act_severity"),
                individual_role=case.get("individual_role"),
