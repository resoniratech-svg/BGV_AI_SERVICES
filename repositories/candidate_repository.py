...
class CandidateRepository:
    ...
    def save_candidate(candidate):
        ...
        values = (
            ...
            candidate.get("total_experience_months"),
            ...
            candidate.get("parsed_json"),
            candidate.get("raw_resume_text"),
            "OLLAMA",
            "SUCCESS",
        )
        ...
        connection.commit()
        parsed_candidate_id = cursor.lastrowid
    ...
    @staticmethod
    def save_raw_resume_data(candidate_id, raw_data):
        connection = get_connection()
        cursor = connection.cursor()
        ...
        values = (candidate_id, json.dumps(raw_data))
        cursor.execute(query, values)
        connection.commit()
        cursor.close()
        connection.close()
    ...
    def save_resume_api_log(
        candidate_id, api_provider, request_payload, response_payload, status
    ):
        connection = get_connection()
        cursor = connection.cursor()
